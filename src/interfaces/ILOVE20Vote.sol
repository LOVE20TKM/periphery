...
interface ILOVE20VoteEvents {
    event Vote(
        address indexed tokenAddress, uint256 round, address indexed voter, uint256 indexed actionId, uint256 votes
    );
}
...
interface ILOVE20Vote is IPhase, ILOVE20VoteErrors, ILOVE20VoteEvents {
    ...
    function votesNumByActionId(address tokenAddress, uint256 round, uint256 actionId)
        ...
        view
        returns (uint256);
    function votesNumByAccount(address tokenAddress, uint256 round, address account) external view returns (uint256);
    function votesNumByAccountByActionId(address tokenAddress, uint256 round, address account, uint256 actionId)
        external
        view
        returns (uint256);

    function vote(address tokenAddress, uint256[] calldata actionIds, uint256[] calldata votes) external;

    function canVote(address tokenAddress, address account) external view returns (bool);

    function maxVotesNum(address tokenAddress, address account) external view returns (uint256);

    function isActionIdVoted(address tokenAddress, uint256 round, uint256 actionId) external view returns (bool);
